...
const filteredTable = computed(() =>
  profileData.user_sessions.filter(
    (row) =>
      row.browser.includes(filter.value) ||
      row.country.includes(filter.value) ||
      row.ip.includes(filter.value) ||
      row.time.includes(filter.value)
  )
)
...
<template>
  <main>
    <section class="user-info">
      <img
        v-bind:src="'data:image/png;base64,' + profileData.user_basics.thumbnail"
        alt="Avatar"
        width="100"
        height="100"
      />
      <p>
        {{ profileData.user_basics.first.toUpperCase() }}
        {{ profileData.user_basics.last.toUpperCase() }}
      </p>
    </section>
    <section>
